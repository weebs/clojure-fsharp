...
let update message model =
    ...
    | Compile text ->
        ...
        | FParsec.CharParsers.ParserResult.Success(tokens, _, _) ->
            let expr = tokens |> List.map (fun ((start, expr), src) -> expr, runtime.Eval expr)
            { model with Source = text; Program = tokens |> List.map (fst >> snd); Expr = expr }
...
type MyApp() =
    ...
    override this.Program =
        ...
        Program.mkSimple (fun _ -> { Source = program; Program = parsed |> List.map (fst >> snd); Output = []; Expr = [] }) update view
